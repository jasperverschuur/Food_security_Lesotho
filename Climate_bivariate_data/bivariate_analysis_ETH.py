...
for i in range(0,382):
    f = nc.MFDataset('JFM_ETH_hist_L_ACT.nc')
    ...
    f = nc.MFDataset('JFM_ETH_hist_SA_ACT.nc')
...
for i in range(0,382):
    f = nc.MFDataset('JFM_ETH_hist_L_NAT.nc')
    ...
    f = nc.MFDataset('JFM_ETH_hist_SA_NAT.nc')
...
for j in range(2000):
    ...
    for i in range(len(standard)):
        val_L_event = (388 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_L_low = (383 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_L_up = (392 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_SA_event = (362 - np.mean(values_SA))/np.std(values_SA)
        val_SA_low = (356 - np.mean(values_SA))/np.std(values_SA)
        val_SA_up = (368 - np.mean(values_SA))/np.std(values_SA)
...
plt.savefig('prob_density_ETH.pdf')
...
plt.xlim(0,1.0)
plt.tight_layout()
plt.savefig('corr_sub_ETH.pdf',transparent = True)
...
ax.plot(388, 362, 'ko')
ax.plot([388,388], [356, 368], 'k')
ax.plot([383, 392],[362, 362],'k')
...
plt.tick_params(axis='both', which='major', labelsize= '12')
plt.tight_layout()
plt.savefig('correlation_empirical_ETH.pdf', orientation='portrait', format='pdf',transparent = True)
...
ax.plot([388,388], [356, 368], 'k')
ax.plot([383, 392],[362, 362],'k')
ax.text(1000,1100,'$\\rho$ = 0.71',color = 'r', fontsize = '12')
ax.text(1000,1060,'$\\rho$ = 0.71',color = 'b', fontsize = '12')
plt.minorticks_on()

plt.legend(['ACT', 'NAT'], markerscale=3., loc = 'lower right', frameon=False)
plt.title('Joint probability plot ACT vs NAT ETH-CAM5')
plt.tick_params(
...
plt.tight_layout()
plt.savefig('correlation_ETH.pdf', orientation='portrait', format='pdf')
...
ax.plot([388,388], [356, 368], 'k')
ax.plot([383, 392],[362, 362],'k')
ax.text(1200,1100,'$\\rho$ = 0.85',color = 'r', fontsize = '12')
ax.text(1200,1060,'$\\rho$ = 0.86',color = 'b', fontsize = '12')
plt.minorticks_on()
plt.tick_params(
    axis='x',          # changes apply to the x-axis
    which='both',      # both major and minor ticks are affected
