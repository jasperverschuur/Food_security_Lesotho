...
for i in range(0,100):
    f = nc.MFDataset('JFM_MIROC5_hist_L_ACT.nc')
    value_les = f.variables['pr'][:]*90
    value_les = np.squeeze(value_les[:,i,0,0])
    values_Lesotho= np.concatenate([values_Lesotho, value_les])

    f.close

    f = nc.MFDataset('JFM_MIROC5_hist_SA_ACT.nc')
    value_SA = f.variables['pr'][:]*90
    value_SA = np.squeeze(value_SA[:,i,0,0])
    values_SA = np.concatenate([values_SA, value_SA])
    f.close

for i in range(0,100):
    f = nc.MFDataset('JFM_MIROC5_hist_L_NAT.nc')
    value_lesN = f.variables['pr'][:]*90
    value_lesN = np.squeeze(value_lesN[:,i,0,0])
    values_LesothoN= np.concatenate([values_LesothoN, value_lesN])

    f.close

    f = nc.MFDataset('JFM_MIROC5_hist_SA_NAT.nc')
    value_SAN = f.variables['pr'][:]*90
    value_SAN = np.squeeze(value_SAN[:,i,0,0])
...
for j in range(2000):
    ...
    for i in range(len(standard)):
        val_L_event = (280 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_L_low = (270 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_L_up = (290 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_SA_event = (242 - np.mean(values_SA))/np.std(values_SA)
        val_SA_low = (232.7 - np.mean(values_SA))/np.std(values_SA)
        val_SA_up = (252.5 - np.mean(values_SA))/np.std(values_SA)
...
#plt.tight_layout()
plt.savefig('prob_density_MIROC.pdf')
...
plt.xlim(0,1.0)
plt.tight_layout()
plt.savefig('corr_sub_MIROC.pdf',transparent = True)
...
ax.plot(280, 242, 'ko')
ax.plot([280,280], [232.7, 252.5], 'k')
ax.plot([270, 290],[242, 242],'k')
...
plt.tick_params(axis='both', which='major', labelsize= '12')
plt.tight_layout()
plt.savefig('correlation_empirical_MIROC.pdf', orientation='portrait', format='pdf',transparent = True)

x, y = np.mgrid[0:1100:1, 0:1100:1]
pos = np.dstack((x, y))
...
x1,y1 = np.mgrid[50:350:0.5, 100:350:0.5]
posnew = np.dstack((x1,y1))


levels = [0.01,0.05, 0.10, 0.50, 0.90,0.95,0.99]
levels1 = [0.88,0.90, 0.92, 0.94, 0.96,0.97, 0.98,0.985, 0.986,0.987,0.988,0.989, 0.99,0.991, 0.992, 0.993, 0.994,0.995, 0.996, 0.997, 0.998, 0.999]
...
ax.plot([280,280], [232.7, 252.5], 'k')
ax.plot([270, 290],[242, 242],'k')
ax.text(1000,1100,'$\\rho$ = 0.72',color = 'r', fontsize = '12')
ax.text(1000,1060,'$\\rho$ = 0.68',color = 'b', fontsize = '12')
plt.minorticks_on()

plt.legend(['ACT', 'NAT'], markerscale=3., loc = 'lower right', frameon=False)
plt.title('Joint probability plot ACT vs NAT MIROC5')
plt.tick_params(
...
plt.tight_layout()
plt.savefig('correlation_MIROC.pdf', orientation='portrait', format='pdf')
...
plt.xlim(200,350)
plt.ylim(200,350)
plt.xlabel('Total precipitation JFM Lesotho (mm)', fontsize = '10')
plt.ylabel('Total precipitation JFM South Africa (mm)',fontsize = '10')
