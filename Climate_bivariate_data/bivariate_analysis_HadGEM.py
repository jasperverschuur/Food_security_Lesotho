...
for i in range(0,15):
    f = nc.MFDataset('pr_Amon_HadGEM3-A-N216_historical_r1i1p10_196001-201312_L_JFM.nc')
    value_les = f.variables['pr'][:]
    value_les = np.squeeze(value_les[:,i,0,0])*86400*90
    values_Lesotho= np.concatenate([values_Lesotho, value_les])

    f.close

    f = nc.MFDataset('pr_Amon_HadGEM3-A-N216_historical_r1i1p10_196001-201312_SA_JFM.nc')
    value_SA = f.variables['pr'][:]
    value_SA = np.squeeze(value_SA[:,i,0,0])*86400*90
    values_SA = np.concatenate([values_SA, value_SA])
    f.close

for i in range(0,15):
    f = nc.MFDataset('pr_Amon_HadGEM3-A-N216_historicalNAT_r1i1p10_196001-201312_L_JFM.nc')
    value_lesN = f.variables['pr'][:]
    value_lesN = np.squeeze(value_lesN[:,i,0,0])*86400*90
    values_LesothoN= np.concatenate([values_LesothoN, value_lesN])

    f.close

    f = nc.MFDataset('pr_Amon_HadGEM3-A-N216_historicalNAT_r1i1p10_196001-201312_SA_JFM.nc')
    value_SAN = f.variables['pr'][:]
    value_SAN = np.squeeze(value_SAN[:,i,0,0])*86400*90
...
print(values_Lesotho[0:10],values_LesothoN[0:10],values_SA[0:10],values_SAN[0:10])
...
for j in range(2000):
    ...
    for i in range(len(standard)):
        val_L_event = (403 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_L_low = (393 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_L_up = (414 - np.mean(values_Lesotho))/np.std(values_Lesotho)
        val_SA_event = (255 - np.mean(values_SA))/np.std(values_SA)
        val_SA_low = (245 - np.mean(values_SA))/np.std(values_SA)
        val_SA_up = (264 - np.mean(values_SA))/np.std(values_SA)
...
plt.savefig('prob_density_HAD.pdf')
...
plt.xlim(0,1.0)
plt.tight_layout()
plt.savefig('corr_sub_HAD.pdf',transparent = True)
...
#ax.clabel(cset, inline=1, fontsize=9,fmt = '%1.2f',colors = 'r')
#ax.clabel(cset1, inline=1, fontsize=9,fmt = '%1.2f',colors = 'b')
ax.set_xlabel('JFM $P_{tot}$ L (mm)',fontsize = '13')
ax.set_ylabel('JFM $P_{tot}$ SA (mm)',fontsize = '13')

ax.plot(403, 255, 'ko')
ax.plot([403,403], [245, 264], 'k')
ax.plot([393, 414],[255, 255],'k')
...
plt.tick_params(axis='both', which='major', labelsize= '12')
plt.tight_layout()
plt.savefig('correlation_empirical_HAD.pdf', orientation='portrait', format='pdf',transparent = True)
...
ax.plot(values_Lesotho,values_SA,'ko',markersize='1.0', alpha = 0.3)
ax.plot(values_LesothoN,values_SAN,'ko',markersize='1.0', alpha = 0.3)
#ax.imshow(np.rot90(f), cmap='coolwarm', extent=[xmin, xmax, ymin, ymax])
cset = ax.contour(xx, yy, f, colors='r')
cset1 = ax.contour(xx1, yy1, f1, colors='b')
#ax.clabel(cset, inline=0, fontsize=10)
#ax.clabel(cset1, inline=0, fontsize=10)
ax.set_xlabel('X')
ax.set_ylabel('Y')
...
x1,y1 = np.mgrid[50:500:0.5, 100:350:0.5]
posnew = np.dstack((x1,y1))


levels = [0.01,0.05, 0.10, 0.50, 0.90,0.95,0.99]
levels1 = [0.88,0.90, 0.92, 0.94,0.945,0.95,0.955, 0.96,0.97, 0.98,0.985, 0.99,0.991, 0.992, 0.993, 0.994,0.995, 0.996, 0.997, 0.998, 0.999]
...
print('corr smaller',np.corrcoef(data_small['val-L'],data_small['val-SA']))
print('corr smaller',np.corrcoef(data_small1['val-L'],data_small1['val-SA']))
...
#plt.title('Joint probability plot ACT vs NAT weather@home')
plt.tick_params(
...
plt.tight_layout()
plt.savefig('correlation_had.pdf', orientation='portrait', format='pdf')
